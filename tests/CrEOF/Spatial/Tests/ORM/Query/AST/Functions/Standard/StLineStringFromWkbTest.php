...
class StLineStringFromWkbTest extends OrmTestCase
{
    ...
    public function testSelect()
    {
        ...
        $query = $this->getEntityManager()->createQuery(
            'SELECT g, St_AsText(ST_LineStringFromWkb(St_AsBinary(g.geometry))) FROM CrEOF\Spatial\Tests\Fixtures\GeometryEntity g'
        );
        // phpcs:enable
    ...
    public function testSelectWithSrid()
    {
        ...
        // phpcs:disable Generic.Files.LineLength.MaxExceeded
        $query = $this->getEntityManager()->createQuery(
            'SELECT g, ST_SRID(ST_LineStringFromWkb(:wkb, :srid)) FROM CrEOF\Spatial\Tests\Fixtures\GeometryEntity g'
        );
