...
class SpMbrDisjointTest extends OrmTestCase
{
    ...
    public function testMbrDisjointWhereParameter()
    {
        ...
        // phpcs:disable Generic.Files.LineLength.MaxExceeded
            'SELECT p FROM CrEOF\Spatial\Tests\Fixtures\PolygonEntity p WHERE MySql_MBRDisjoint(p.polygon, ST_GeomFromText(:p)) = 1'
        // phpcs:enable
        );
    ...
    public function testSelectMbrDisjoint()
    {
        ...
        $query = $this->getEntityManager()->createQuery(
            // phpcs:disable Generic.Files.LineLength.MaxExceeded
            'SELECT p, MySql_MBRDisjoint(p.polygon, ST_GeomFromText(:p1)) FROM CrEOF\Spatial\Tests\Fixtures\PolygonEntity p'
            // phpcs:enable
        );
