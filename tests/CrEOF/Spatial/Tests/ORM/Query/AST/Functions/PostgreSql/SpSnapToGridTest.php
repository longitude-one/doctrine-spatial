...
class STSnapToGridTest extends OrmTestCase
{
    ...
    public function testSelectStSnapToGridSignature2Parameters()
    {
        ...
        $query = $this->getEntityManager()->createQuery(
            'SELECT ST_AsText(PgSql_SnapToGrid(p.point, 0.5)) FROM CrEOF\Spatial\Tests\Fixtures\PointEntity p'
        );
    ...
    public function testSelectStSnapToGridSignature3Parameters()
    {
        ...
        $query = $this->getEntityManager()->createQuery(
            ...
            'SELECT ST_AsText(PgSql_SnapToGrid(p.point, 0.5, 1)) FROM CrEOF\Spatial\Tests\Fixtures\PointEntity p'
            // phpcs:enable
        );
        $result = $query->getResult();
    ...
    public function testSelectStSnapToGridSignature5Parameters()
    {
        ...
        $query = $this->getEntityManager()->createQuery(
        // phpcs:disable Generic.Files.LineLength.MaxExceeded
            'SELECT ST_AsText(PgSql_SnapToGrid(p.point, 5.55, 6.25, 0.5, 0.5)) FROM CrEOF\Spatial\Tests\Fixtures\PointEntity p'
        // phpcs:enable
        );
        $result = $query->getResult();
    ...
    public function testSelectStSnapToGridSignature6Parameters()
    {
        ...
        $query = $this->getEntityManager()->createQuery(
        // phpcs:disable Generic.Files.LineLength.MaxExceeded
            'SELECT ST_AsText(PgSql_SnapToGrid(p.point, p.point, 0.005, 0.025, 0.5, 0.01)) FROM CrEOF\Spatial\Tests\Fixtures\PointEntity p'
        // phpcs:enable
        );
        $result = $query->getResult();
