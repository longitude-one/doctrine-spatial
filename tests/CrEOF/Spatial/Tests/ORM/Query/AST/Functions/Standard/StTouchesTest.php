...
class StTouchesTest extends OrmTestCase
{
    ...
    public function testFunctionInPredicate()
    {
        ...
        $query = $this->getEntityManager()->createQuery(
            ...
            'SELECT p FROM CrEOF\Spatial\Tests\Fixtures\PolygonEntity p WHERE ST_Touches(p.polygon, ST_GeomFromText(:p)) = true'
            // phpcs:enable
        );
    ...
    public function testFunctionInSelect()
    {
        ...
        $query = $this->getEntityManager()->createQuery(
            // phpcs:disable Generic.Files.LineLength.MaxExceeded
            'SELECT ST_Touches(p.polygon, ST_GeomFromText(:p)) FROM CrEOF\Spatial\Tests\Fixtures\PolygonEntity p'
            // phpcs:enable
        );
        $query->setParameter('p', 'LINESTRING(0 0, 0 10)', 'string');
        $result = $query->getResult();
