...
class StDisjointTest extends OrmTestCase
{
    ...
    public function testSelectStDisjoint()
    {
        ...
        $query = $this->getEntityManager()->createQuery(
            'SELECT p, ST_Disjoint(p.polygon, ST_GeomFromText(:p1)) FROM CrEOF\Spatial\Tests\Fixtures\PolygonEntity p'
        );
    ...
    public function testStDisjointWhereParameter()
    {
        ...
        $query = $this->getEntityManager()->createQuery(
            ...
            'SELECT p FROM CrEOF\Spatial\Tests\Fixtures\PolygonEntity p WHERE ST_Disjoint(p.polygon, ST_GeomFromText(:p1)) = true'
            // phpcs:enable
        );
