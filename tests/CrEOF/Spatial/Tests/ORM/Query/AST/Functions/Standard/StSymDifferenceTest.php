...
class StSymDifferenceTest extends OrmTestCase
{
    ...
    public function testSelectStDifference()
    {
        ...
        $query = $this->getEntityManager()->createQuery(
            ...
            'SELECT l, ST_AsText(ST_SymDifference(ST_GeomFromText(:p), l.lineString)) FROM CrEOF\Spatial\Tests\Fixtures\LineStringEntity l'
            // phpcs:enable
        );
    ...
    public function testStDifferenceWhereParameter()
    {
        ...
        $query = $this->getEntityManager()->createQuery(
            // phpcs:disable Generic.Files.LineLength.MaxExceeded
            'SELECT l FROM CrEOF\Spatial\Tests\Fixtures\LineStringEntity l WHERE ST_IsEmpty(ST_SymDifference(ST_GeomFromText(:p1), l.lineString)) = false'
            // phpcs:enable
        );
