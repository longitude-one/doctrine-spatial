...
class StWithinTest extends OrmTestCase
{
    ...
    public function testFunctionInPredicate()
    {
        ...
        $query = $this->getEntityManager()->createQuery(
            ...
            'SELECT p FROM CrEOF\Spatial\Tests\Fixtures\PolygonEntity p WHERE ST_Within(p.polygon, ST_GeomFromText(:p)) = true'
            // phpcs:enable
        );
    ...
    public function testFunctionInSelect()
    {
        ...
        $query = $this->getEntityManager()->createQuery(
            'SELECT p, ST_Within(p.polygon, ST_GeomFromText(:p)) FROM CrEOF\Spatial\Tests\Fixtures\PolygonEntity p'
        );
