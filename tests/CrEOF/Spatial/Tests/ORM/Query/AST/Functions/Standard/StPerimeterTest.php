 ...
 * @coversDefaultClass
 */
class StPerimeterTest extends OrmTestCase
{
    ...
    public function testFunctionInPredicate()
    {
        ...
        $query = $this->getEntityManager()->createQuery(
            ...
            'SELECT p FROM CrEOF\Spatial\Tests\Fixtures\PolygonEntity p WHERE ST_Perimeter(p.polygon) = :p'
            // phpcs:enable
        );
    ...
    public function testFunctionInSelect()
    {
        ...
        $query = $this->getEntityManager()->createQuery(
            'SELECT p, ST_Perimeter(p.polygon) FROM CrEOF\Spatial\Tests\Fixtures\PolygonEntity p'
        );
        $result = $query->getResult();
