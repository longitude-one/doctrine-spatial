...
class StMPolyFromWkbTest extends OrmTestCase
{
    ...
    public function testSelect()
    {
        ...
        $query = $this->getEntityManager()->createQuery(
            'SELECT t, ST_AsText(ST_MPolyFromWkb(:wkb)) FROM CrEOF\Spatial\Tests\Fixtures\GeometryEntity t'
        );
        // phpcs:disable Generic.Files.LineLength.MaxExceeded
    ...
    public function testSelectWithSrid()
    {
        ...
        $query = $this->getEntityManager()->createQuery(
            'SELECT t, ST_SRID(ST_MPolyFromWkb(:wkb, :srid)) FROM CrEOF\Spatial\Tests\Fixtures\GeometryEntity t'
        );
        $query->setParameter('wkb', hex2bin(self::DATA), 'blob');
