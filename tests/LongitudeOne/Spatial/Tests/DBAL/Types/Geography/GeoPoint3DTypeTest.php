...
use LongitudeOne\Spatial\DBAL\Types\Geography\Point3DType;
 ...
 */
class GeoPoint3DTypeTest extends PersistOrmTestCase
{
    ...
    {
        $this->usesEntity(self::GEO_POINT_3D_ENTITY);
    ...
    public function testDeclarationType(): void
    {
        ...
        $metadata = $this->getEntityManager()->getClassMetadata(GeoPoint3DEntity::class);
        ...
        static::assertEquals('geopoint3d', $type);
    }
    ...
    public function testFindByPoint(): void
    {
        ...
        $entity = new GeoPoint3DEntity();

        $entity->setPoint($point);
        $this->getEntityManager()->persist($entity);
        $this->getEntityManager()->flush();

        $result = $this->getEntityManager()->getRepository(self::GEO_POINT_3D_ENTITY)->findByPoint($point);
    ...
    {
        if (!Type::hasType('geopoint3d')) {
            Type::addType('geopoint3d', Point3DType::class);
        }

        $spatialInstance = new Point3DType();
        static::assertNotFalse($spatialInstance->getName());
        static::assertSame('geopoint3d', $spatialInstance->getName());
    ...
    {
        $pointEntity = new GeoPoint3DEntity();
