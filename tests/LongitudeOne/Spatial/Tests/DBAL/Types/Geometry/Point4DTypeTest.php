...
use LongitudeOne\Spatial\PHP\Types\Geometry\Point4D;
 ...
 */
class Point4DTypeTest extends PersistOrmTestCase
{
    ...
    {
        $this->usesEntity(self::POINT4D_ENTITY);
        $metadata = $this->getEntityManager()->getClassMetadata(Point4DEntity::class);
        ...
        static::assertEquals('point4d', $type);
    }
    ...
    public function testFindByPoint(): void
    {
        ...
        $entity = new Point4DEntity();

        $entity->setPoint($point);
        $this->getEntityManager()->persist($entity);
        $this->getEntityManager()->flush();

        $result = $this->getEntityManager()->getRepository(self::POINT4D_ENTITY)->findByPoint($point);
    ...
    {
        if (!Type::hasType('point4d')) {
            Type::addType('point4d', Point4DType::class);
        }

        $spatialInstance = new Point4DType();
        static::assertNotFalse($spatialInstance->getName());
        static::assertSame('point4d', $spatialInstance->getName());
